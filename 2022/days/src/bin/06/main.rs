...
mod tests {
    ...
    fn first_start_of_packet_offset_4() {
        ...
        assert_eq!(
            Some(7),
            first_start_of_packet("mjqjpqmgbljsphdztnvjfqwrcgsmlb", OFFSET)
        );
        assert_eq!(
            Some(5),
            first_start_of_packet("bvwbjplbgvbhsrlpgdmjqwftvncz", OFFSET)
        );
        assert_eq!(
            Some(6),
            first_start_of_packet("nppdvjthqldpwncqszvftbrmjlhg", OFFSET)
        );
        assert_eq!(
            Some(10),
            first_start_of_packet("nznrnfrfntjfmvfwmzdfjlvtqnbhcprsg", OFFSET)
        );
        assert_eq!(
            Some(11),
            first_start_of_packet("zcfzfwzzqfrljwzlrfnpqdbhtmscgvjw", OFFSET)
        );
    }
